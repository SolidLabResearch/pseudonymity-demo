...
describe('WebIdOnWebIdActor', (): void => {
    ...
    const actorFactory = new WebIdOnWebIdActorFactory(defaultDocumentLoaderCacheOptions)
    let actor: WebIdOnWebIdActor
    async function createInitializedActor(): Promise<WebIdOnWebIdActor> {
    ...
    it('Can create, sign, and verify a VP using the public actor, and verify with both', async() => {
        actor.enablePublicActor()
    ...
    it('Can create, sign, and verify a VP using the pseudonymous actor, and verify with both', async() => {
        actor.enablePseudonymousActor()
        const vc = await actor.signCredential(
            actor.createCredential({
                'id': 'urn:test',
    ...
    it('Can derive a VC using the pseudonymous actor', async () => {
        ...
        expect(dvc.proof.verificationMethod).toStrictEqual(actor.pseudonymousActor.key.id)
    })
    ...
    it('Can create and sign a VP with a derived VC using the pseudonymous actor, and verify with both', async () => {
        ...
        // Verify (w. pseudonymous WebID actor)
        actor.enablePseudonymousActor()
        const vr1 = await actor.verifyPresentation(vp, challenge)
        if(vr1.verified !== true)
            console.error(vr1)
